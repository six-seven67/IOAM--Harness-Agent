...
from app.services.vector.embedding import vector_embedding_service
...
class VectorStoreManager:
    ...
    def get_source_hash(self, file_path: str) -> str | None:
        """
        查询 Milvus 中指定文件的内容哈希值（用于增量索引判断）。

        查询 _source 等于 file_path 的任意一条文档，
        读取其 metadata 中的 _content_hash 字段。

        Args:
            file_path: 文件路径

        Returns:
            str | None: 上次索引时存储的内容哈希，未找到返回 None
        """
        try:
            collection = milvus_manager.get_collection()
            expr = f'metadata["_source"] == "{file_path}"'
            ...
            if results and len(results) > 0:
                ...
                return meta.get("_content_hash") if isinstance(meta, dict) else None
            return None
        except Exception as e:
            logger.warning(f"查询文件哈希失败 (可能是首次索引): {e}")
            return None
