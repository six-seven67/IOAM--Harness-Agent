...
from app.services.vector.splitter import document_splitter_service
...
class VectorIndexService:
    ...
    def index_directory(self, directory_path: Optional[str] = None) -> IndexingResult:
        ...
        try:
            ...
            for file_path in files:
                try:
                    status = self.index_single_file(str(file_path))
                    if status == "skipped":
            ...
            logger.info(
                ...
                f"已索引={result.success_count}, 跳过={result.skipped_count}, 失败={result.fail_count}"
            )
    ...
    def index_single_file(self, file_path: str) -> str:
        """
        索引单个文件 (使用新的 LangChain 分割器)

        增量索引: 如果文件内容未变化 (MD5 相同), 跳过重新嵌入。

        Args:
            file_path: 文件路径

        Returns:
            str: "indexed" — 新建/更新索引; "skipped" — 内容未变化，跳过索引
        ...
        try:
            ...
            logger.info(f"读取文件: {path}, 内容长度: {len(content)} 字符, MD5: {content_hash}")

            # 2. 检查是否已索引且内容未变化（增量索引）
            normalized_path = path.as_posix()
            stored_hash = vector_store_manager.get_source_hash(normalized_path)
            ...
            if stored_hash is not None:
                logger.info(
                    f"文件内容已变化: {file_path} "
                    f"(旧={stored_hash[:8]}..., 新={content_hash[:8]}...)"
                )
